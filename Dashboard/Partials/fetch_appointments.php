...
$sqlTotal = "SELECT COUNT(*) AS total FROM appointments";
$resultTotal = $conn->query($sqlTotal);
$totalRows = $resultTotal->fetch_assoc()['total'];
$totalPages = ceil($totalRows / $rowsPerPage);

$sql = "SELECT id, reference_code FROM appointments LIMIT $offset, $rowsPerPage";
...
if ($result->num_rows > 0) {
    while ($row = $result->fetch_assoc()) {
        ...
        $rows .= "<td class='action-buttons'>
                    <button class='view-button' onclick='viewDetails(" . $row["id"] . ", \"appointments\")'>View</button>
